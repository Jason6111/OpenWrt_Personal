...
# 默认语言中文，其他英文
echo -e "\033[37m 请选择默认语言，输入任意字符为英文，不输入默认中文\033[0m"
echo -e "\033[37m Please select the default language, enter any character as English, and do not enter the default Chinese. \033[0m"
...
LogMessage "\033[37m 是否安装编译依赖，不输入默认不安装，输入任意值安装，将会在$timer秒后自动选择默认值 \033[0m" "\033[37m Whether to install the compilation dependencies. Do not enter the default. Do not install. Enter any value to install. The default value will be automatically selected after $timer seconds \033[0m"
    ...
    if [ ! -n "$dependencies" ]; then
        ...
        LogMessage "\033[31m 开始安装 \033[0m" "\033[31m Start installation \033[0m"
        sudo apt-get update
        sleep 10s
...
function DIY_Script(){
    ...
    sleep 1s
    # Modify default IP
    # LogMessage "\033[37m 设置路由默认地址为10.10.0.253 \033[0m" "\033[37m Set the route default address to 10.10.0.253 \033[0m"
...
function DIY_Script1(){
    #id
    LogMessage "\033[37m 显示固件作者，不输入默认不显示，输入任意值显示（只需一次），将会在$timer秒后自动选择默认值 \033[0m" "\033[37m Show firmware author，No input, no display by default, enter any value to display (only once).The default value will be automatically selected after $timer seconds \033[0m"
    read -t $timer ID
    if [ ! -n "$ID" ]; then
...
function DIY_Script2(){
    #id
    LogMessage "\033[37m 显示DIY主题背景，不输入默认不显示，输入任意值显示（只需一次），将会在$timer秒后自动选择默认值 \033[0m" "\033[37m Display the DIY theme background, do not enter the default, enter any value to display (only once), and the default value will be automatically selected after $timer seconds \033[0m"
    read -t $timer ID
    if [ ! -n "$ID" ]; then
	LogMessage "\033[32m OK，不显示DIY主题背景 \033[0m" "\033[32m OK, DIY theme background is not displayed \033[0m"
    else
        LogMessage "\033[31m 显示DIY主题背景 \033[0m" "\033[31m Show DIY theme background \033[0m"
...
function Compile_Firmware() {
    ...
    LogMessage "\033[31m 创建编译日志文件夹/home/${userName}/${log_folder_name}/${folder_name} \033[0m" "\033[31m Create compilation log folder /home/${userName}/${log_folder_name}/${folder_name} \033[0m"
    sleep 1s
    ...
    LogMessage "\033[32m 编译日志文件夹创建成功 \033[0m" "\033[32m The compilation log folder was created successfully \033[0m"
    sleep 1s
    LogMessage "\033[31m 开始编译！！ \033[0m" "\033[31m Start compiling! ! \033[0m"
    sleep 1s
...
Get_luci_apps
    ...
    #     echo -e  "\033[37m 由于你是首次编译，需要make menuconfig配置，如果保持原有config不做更改，请在进入菜单后直接exit即可 \033[0m"
    #     sleep 6s
    #     make menuconfig
    # fi
    # if [[ $isFirstCompile == 0 ]]; then
    #     echo -e  "\033[37m 开始执行make defconfig! \033[0m"
    #     make defconfig | tee -a /home/${userName}/${log_folder_name}/${folder_name}/Main1_make_defconfig-git_log.log

    # fi

    LogMessage "\033[31m 开始执行make defconfig! \033[0m" "\033[31m Start to execute make defconfig! \033[0m"
    sleep 1s
    cd /home/${userName}/${ledeDir}/
    ...
    LogMessage "\033[31m 开始执行make download! \033[0m" "\033[31m Start to execute make download! \033[0m"
    sleep 1s
    ...
    then
        ...
        if [ ! -n "$isSingleCompile" ]; then
            LogMessage "\033[32m OK，不执行单线程编译  \033[0m" "\033[32m OK, do not perform single-threaded compilation  \033[0m"
            sleep 1s
            ...
            LogMessage "\033[32m OK，执行单线程编译。 \033[0m" "\033[32m OK, execute single-threaded compilation. \033[0m"
            LogMessage "\033[31m 准备开始编译 \033[0m" "\033[31m Ready to compile \033[0m"
            sleep 1s
    ...
    LogMessage "\033[32m make编译结束! \033[0m" "\033[32m Make compilation is over! \033[0m"
    sleep 1s
    ...
    read -t $timer iscopy
    if [ ! -n "$iscopy" ]; then
        LogMessage "\033[31m 开始拷贝 \033[0m" "\033[31m Start copying \033[0m"
        cp -r /home/${userName}/${ledeDir}/bin/targets /home/${userName}/${log_folder_name}/${folder_name}
        LogMessage "\033[32m 拷贝完成 \033[0m" "\033[32m Copy completed \033[0m"    
    else
        LogMessage "\033[32m OK，不拷贝 \033[0m" "\033[32m OK, don't copy \033[0m"
    fi

    # 将lede还原
    LogMessage "\033[37m 将lede源码还原到最后的hash状态! \033[0m" "\033[37m Restore the lede source code to the last hash state \033[0m"
...
function CleanLogFolder(){
    ...
    then
        ...
        if [ ! -n "$isclean" ]; then
            ...
            LogMessage "\033[32m 清理成功 \033[0m" "\033[32m Cleaned up successfully \033[0m"
        else
            LogMessage "\033[32m OK，不清理超过$clean_day天的日志文件 \033[0m" "\033[32m OK, do not clean up log files older than $clean_day \033[0m"
        fi
    fi
}
...
if [ ! -n "$isCreateNewConfig" ]; then
    ...
    LogMessage "\033[37m 请输入新的Config文件名，请以xxx.config命名，例如xiaomi3.config \033[0m" "\033[37m Please enter the new Config file name, please name it after xxx.config, for example xiaomi3.config \033[0m"
    read newConfigName
    for conf in ${config_list[*]}; 
    do 
    ...
    do
        LogMessage "\033[31m 你输入的值为空或者与现有config文件名重复,请重新输入！ \033[0m" "\033[31m The value you entered is empty or duplicates the name of the existing config file, please re-enter! \033[0m"
        read  newConfigName
        for conf in ${config_list[*]}; 
        do 
        if [[ $newConfigName = $conf ]]; then
...
if [ ! -n "$isCreateNewConfig" ]; then
    echo
    LogMessage "\033[31m 请选择OpenWrt_Personal中的config文件，默认为$configName \033[0m" "\033[31m Please select the config file in OpenWrt_Personal, the default is $configName \033[0m"
    LogMessage "\033[37m 将会在$timer秒后自动选择默认值 \033[0m" "\033[37m The default value will be automatically selected after $timer seconds \033[0m"
    configList
    until [[ $configNameInp -ge 1 && $configNameInp -le $key ]]
    do
        LogMessage "\033[37m 你输入的 ${configNameInp} 是啥玩应啊，看好了序号，输入数值就行了。 \033[0m" "\033[37m What is the function of the ${configNameInp} you entered? Just take a good look at the serial number and just enter the value. \033[0m"
        LogMessage "\033[31m 请选择OpenWrt_Personal中的config文件，默认为$configName \033[0m" "\033[31m Please select the config file in OpenWrt_Personal, the default is $configName \033[0m"
        configList
    done
    ...
    if [ ! -n "$ledeDirInp" ]; then
        LogMessage "\033[32m OK，使用默认值$ledeDir \033[0m" "\033[32m OK, use the default value $ledeDir \033[0m"
    else
        LogMessage "\033[37m 使用 ${ledeDirInp} 作为lean源码文件夹名。 \033[0m" "\033[37m Use ${ledeDirInp} as the lean source folder name. \033[0m"
        echo -e  
        ledeDir=$ledeDirInp
    fi
...
echo 
LogMessage "\033[37m 准备就绪，请按照导航选择操作.... \033[0m" "\033[37m Ready, please follow the navigation options... \033[0m"
sleep 2s
...
if [ ! -n "$isCreateNewConfig" ]; then
    ...
    LogMessage "\033[37m 将会在$timer秒后自动选择默认值 \033[0m" "\033[37m The default value will be automatically selected after $timer seconds \033[0m"
    LogMessage "\033[37m 1. 根据config自动编译固件。(默认) \033[0m" "\033[37m 1. Automatically compile the firmware according to config. (default)  \033[0m"
    LogMessage "\033[37m 2. 我要配置config，配置完毕后自动同步回OpenWrt_Personal。 \033[0m" "\033[37m 2. I want to configure config, and automatically Jason6111 back to OpenWrt_Personal after configuration. \033[0m"
    read -t $timer num
    if [ ! -n "$num" ]; then
            num=1
            LogMessage "\033[32m 使用默认值 \033[0m" "\033[32m Use default \033[0m"
    ...
    do
        LogMessage "\033[37m 你输入的 ${num} 是啥玩应啊，看好了序号，输入数值就行了。 \033[0m" "\033[37m What is the function of the ${num} you entered? Just enter the number after you are optimistic about the serial number. \033[0m"
        LogMessage "\033[31m 你接下来要干啥？？？ \033[0m" "\033[31m What are you going to do next? ? ? \033[0m"
        LogMessage "\033[37m 将会在$timer秒后自动选择默认值 \033[0m" "\033[37m The default value will be automatically selected after $timer seconds \033[0m"
        LogMessage "\033[37m 1. 根据config自动编译固件。(默认) \033[0m" "\033[37m 1.Automatically compile the firmware according to config. (default) \033[0m"
        LogMessage "\033[37m 2. 我要配置config，配置完毕后自动同步回OpenWrt_Personal。 \033[0m" "\033[37m 2.I want to configure config, and automatically Jason6111 back to OpenWrt_Personal after configuration. \033[0m"
...
then
    ...
    if [ ! -n "$(git config --global user.email)" ]; then
        ...
        do
            LogMessage "\033[37m 不能输入空值 \033[0m" "\033[37m Cannot enter a null value \033[0m"
            read  gitUserEmail
        done
    ...
    LogMessage "\033[37m 将会在$timer秒后自动选择默认值 \033[0m" "\033[37m The default value will be automatically selected after $timer seconds \033[0m"
    LogMessage "\033[37m 1. 是(默认值) \033[0m" "\033[37m 1.Yes(Default) \033[0m"
    LogMessage "\033[37m 2. 不编译了。退出 \033[0m" "\033[37m 2.NO, Exit \033[0m"
    ...
    do
        LogMessage "\033[37m 你输入的 ${num_continue} 是啥玩应啊，看好了序号，输入数值就行了。 \033[0m" "\033[37m What's the answer for the ${num_continue} you entered? Just enter the number after you are optimistic about the serial number. \033[0m"
        LogMessage "\033[37m 是否根据新的config编译？ \033[0m" "\033[37m Is it compiled according to the new config? \033[0m"
        LogMessage "\033[37m 将会在$timer秒后自动选择默认值 \033[0m" "\033[37m The default value will be automatically selected after $timer seconds \033[0m"
        LogMessage "\033[37m 1. 是(默认值) \033[0m" "\033[37m 1. Yes(Default) \033[0m"
        LogMessage "\033[37m 2. 不编译了。退出  NO, Exit \033[0m" "\033[37m 2.NO, Exit \033[0m"
        read -t $timer num_continue
            if [ ! -n "$num_continue" ]; then
                num_continue=1
                LogMessage "\033[32m 使用默认值 \033[0m" "\033[32m Use default \033[0m"
